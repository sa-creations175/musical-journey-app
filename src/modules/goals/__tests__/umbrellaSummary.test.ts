...
describe('umbrellaSubtitle', () => {
  ...
  it('omits Consistency when no consistency child exists (7e: no append)', () => {
    // Subtitle reflects only dimensions with active child
    // records. Since the consistency dimension stopped
    // generating child goals (7e decision), it correctly
    // disappears from the subtitle for new yearly anchors.
    const children = [
      mkGoal({ targetMetric: 'ear_training_coverage_at_acquired' }),  // Breadth
      mkGoal({ targetMetric: 'ear_training_accuracy_overall' }),      // Accuracy
    ];
    expect(umbrellaSubtitle(children)).toBe('Breadth · Accuracy');
  });
