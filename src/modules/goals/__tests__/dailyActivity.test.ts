...
describe('getDailyActivity — shapes-and-patterns', () => {
  it('aggregates drillSessions duration into minutes per local day', async () => {
    await db.drillSessions.bulkPut([
      { hand: 'both', style: 'solid', id: '1', drillTypeId: 'maj-shape', skillId: 's', durationSeconds: 600, feelRating: 3, timestamp: new Date(2026, 3, 29, 9, 0).getTime() },
      { hand: 'both', style: 'solid', id: '2', drillTypeId: 'min-shape', skillId: 's', durationSeconds: 540, feelRating: 4, timestamp: new Date(2026, 3, 29, 18, 0).getTime() },
    ]);
  ...
  it('rounds minutes per day', async () => {
    await db.drillSessions.bulkPut([
      { hand: 'both', style: 'solid', id: '1', drillTypeId: 'd', skillId: 's', durationSeconds: 90,  feelRating: 3, timestamp: new Date(2026, 3, 10, 9).getTime() }, // 1.5 min
      { hand: 'both', style: 'solid', id: '2', drillTypeId: 'd', skillId: 's', durationSeconds: 105, feelRating: 3, timestamp: new Date(2026, 3, 10, 12).getTime() }, // 1.75
    ]);
  ...
  it('excludes sessions outside the range', async () => {
    await db.drillSessions.bulkPut([
      { hand: 'both', style: 'solid', id: '1', drillTypeId: 'd', skillId: 's', durationSeconds: 600, feelRating: 3, timestamp: new Date(2026, 2, 31, 12).getTime() }, // March 31
      { hand: 'both', style: 'solid', id: '2', drillTypeId: 'd', skillId: 's', durationSeconds: 600, feelRating: 3, timestamp: new Date(2026, 3, 15, 12).getTime() }, // April 15
    ]);
