...
/** Scale types per key in a FIXED musical-pair order — major +
 *  major-pentatonic (major tonality), then natural-minor +
 *  minor-pentatonic (minor tonality), all four in order. The within-key
 *  ordering is fixed regardless of individual item due dates; the KEY
 *  order remains spacing-driven (see pickScalesKeys). The budget walk
 *  in buildScaleLadder truncates the tail when it runs out. */
function pickScaleTypesForKey(): ScaleKind[] {
  return [...SCALES_LADDER];
}
...
function buildScaleLadder(
  ...
  for (const keyName of keys) {
    const kindsForKey = pickScaleTypesForKey();
