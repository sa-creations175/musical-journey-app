/**
 * In-session voice-leading drill runner — Level 3 auto-navigation.
 *
 * The voice-leading counterpart to InSessionDrillRunner (scales) and
 * ChordShapeDrillRunner. On GO, instead of dropping the user at the S&P
 * matrix, this walks the prep-screen per-item breakdown and opens
 * VoiceLeadingDrillModal for each VL sub-cell in turn — pre-seeded with
 * that item's allotted seconds and the prep-screen metronome.
 *
 * Between cells (Next / Previous), a brief prep screen names the next
 * pattern + its drill time; Ready then plays the count-in before the
 * drill starts (BPM/meter carry over from the block prep screen). The
 * FIRST cell skips both — the block prep screen + its count-in handled it.
 *
 * VL sub-cells run off the static catalog (no DB rows), so the cell list
 * resolves synchronously, like the scales runner.
 */
import { useEffect, useMemo, useRef, useState } from 'react';
import VoiceLeadingDrillModal from '../shapes-and-patterns/VoiceLeadingDrillModal';
...
export default function VoiceLeadingDrillRunner({ items, accent, onComplete }: Props) {
  ...
  // Previous): the user sees the next pattern + its drill time and taps
  ...
  // yank us to rating mid-walk. Cleared on unmount.
  useEffect(() => {
    setInSessionDrillActive(true);
    return () => setInSessionDrillActive(false);
  }, [setInSessionDrillActive]);

  // VoiceLeadingDrillModal fires onLogged THEN onClose when a rep is
  // logged, but only onClose on cancel. Without this flag the trailing
  // onClose would end the runner after the first cell instead of
  // advancing. Set on log, consumed by the very next onClose.
  ...
  // Between-cells prep: name the next pattern + its allotted time and
  // wait for Ready before the count-in. BPM/meter carry over from the
  // block prep screen, so there are no controls here — just "what's
  // coming".
  if (betweenPrep) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-8 bg-neutral-950/90 backdrop-blur-sm select-none p-6 text-center">
        <div className="space-y-2">
          ...
          <div className="text-sm text-neutral-400">
            {current.subLabel ? `${current.subLabel} · ` : ''}{formatDuration(current.seconds)}
          </div>
        </div>
        <button
          type="button"
