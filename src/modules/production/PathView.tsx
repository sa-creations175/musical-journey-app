...
import { isCovered, ratingOption } from './lessonRating';
...
/**
 * Lesson list for a single path. Shows each lesson's goal, rating
...
export default function PathView({ pathId, onOpenLesson, onBack }: Props) {
  ...
  // "Covered" is the tried-it line, not the read-it line — the bar
  // tracks lessons the user has actually run.
  const coveredCount = states.filter(s => isCovered(s.rating ?? 0)).length;
  const progressPct = lessons.length === 0 ? 0 : Math.round((coveredCount / lessons.length) * 100);
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <header className="space-y-2">
        ...
        <div className="flex items-center gap-3 pt-1">
          <span className="text-[11px] text-neutral-500">
            {coveredCount} of {lessons.length} lessons tried
          </span>
      ...
      <ul className="rounded-2xl border border-black/[0.07] overflow-hidden divide-y divide-neutral-100 dark:divide-neutral-800">
        {lessons.map(l => {
          ...
          const option = ratingOption(s?.rating);
          ...
          return (
            <li key={l.id}>
              ...
              >
                <span className={`col-span-1 w-2.5 h-2.5 rounded-full shrink-0 ${option.dot}`} aria-hidden />
                ...
                <div className="col-span-3 sm:col-span-2 text-right">
                  <div className="text-[11px] text-neutral-500">{option.label}</div>
                  {revisits > 0 && (
