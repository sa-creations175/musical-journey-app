...
| 23 Aug | 3d-10 | **"Due" surfaced in all three places.** A chip on each song card naming the key, a banner on the song page stating the rule before it acts, and an `N due` pill on the dashboard's repertoire row. A song is due when a key still HOLDING part of its rung is due or due soon — overdue is excluded, because the rung has already dropped and the demotion notice owns that ground. |
...
|---|---|---|
| **A sixth positional-id category in harmonic fluency** | The module-home audit named five categories whose ids carry no grid coordinates — named-notes (`nn-N`), reverse-key-pivots (`rkp-N`), intervals (`iv-N`), tritone-pairs (`tt-N`) and progressions (`pr-N`). **Enharmonic equivalents is a sixth**: `enh-n-N` and `enh-i-N` share one counter. It matters only when the category-detail grid lands and needs an axis; the generator holds the values (`x`/`y` for notes, `m`/`members` for intervals), so supplying them is the same one-line change as the other five. Recorded, deliberately not fixed, while the module-home pattern is still being designed. | `harmonic-fluency/catalog.ts` → the `enh-n-` / `enh-i-` generators |
| **ChordProgressionsQuiz carries focus state with no URL wire-in** | Four ear-training quizzes accept `?focus=` and open already narrowed; chord progressions has `focusKeys` / `focusActive` state but initialises `focusActive` to `false` with no `initialFocusKeys` prop, so a dashboard row tap cannot arm it and the modal is its only entry point. Its `focusProtected` is also behind the others twice over — a literal `4` rather than `FLUENCY_POOL_MINIMUM`, and `focusKeys.length` rather than `new Set(...).size`, which is the exact bug chord recognition's own comment says it fixed. **Not fixed here**: the filter-strip contract will decide how a drill receives a narrowing, and repairing the old path first would mean building the thing twice. | `ChordProgressionsQuiz.tsx` lines 179, 230 · `lib/fluencyPool.ts` for the constant |
