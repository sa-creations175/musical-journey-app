...
### Canonical vocabulary across each cognitive structure (UPDATED — April 2026)
When an app uses defined levels, stages, or categories (proficiency, mastery, freshness, priority, etc.), the same vocabulary applies everywhere within a given cognitive structure of mastery. A "Comfortable" song means the same thing in Song Repertoire, in Goals, on the Dashboard, and in Practice Sessions. Across cognitive structures with genuinely different mastery dynamics (e.g., self-assessed integrated learning vs. measured-accuracy skill-building), vocabulary is harmonized but specifically chosen to reflect each structure's actual progression character. Users encounter related-but-distinct vocabularies, never arbitrary inconsistency. UI surfaces that ask users to engage with these concepts (e.g., goal-setting forms) introduce the vocabulary in the same breath as asking the user to use it.

### Measurable horizons ≤ 1 year (NEW — April 2026)
Goals with concrete measurable targets (target metric, target value, target date) are constrained to horizons of 1 year or less. Longer horizons (multi-year, lifetime) capture qualitative aspiration as text only — no measurable target. Reasoning: meaningful concrete targets at multi-year horizons usually fail honesty checks; they encourage premature concretion of evolving aspirations. The vision layers exist to capture direction; the measurable layers exist to capture commitments. Conflating them weakens both. Applies across all Personal OS apps: fitness goals, financial goals, mental-health goals all follow the same convention.

### Goal feasibility surfaces — substantive design, deferred (NEW — April 2026)
When the system has enough historical data to know the user's typical pace in a domain, it surfaces gentle ambition checks at goal creation ("Last month you completed 2 songs at Comfortable; this month's goal is 5 — want to adjust?") and at goal-period midpoints ("You're 2 weeks in, finished 1 of 5; remaining pace would need to double — adjust target, extend deadline, or push through?"). Both are non-blocking; the user retains agency. This is substantive design that depends on historical pace data and is deferred to a mature phase (Phase 7+ in the music app's roadmap). Capturing the principle now ensures it isn't lost as the data accumulates.
